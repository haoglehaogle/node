...
func (tc *testContext) SetupTest() {
	...
	fakeVpnClientFactory := func(vpnSession session.SessionDto, consumerID identity.Identity, providerID identity.Identity, callback state.Callback, options dto2.ConnectOptions) (openvpn.Process, error) {
		tc.RLock()
		defer tc.RUnlock()
...
func (tc *testContext) TestWithUnknownProviderConnectionIsNotMade() {
	...
	assert.Equal(tc.T(), noProposalsError, tc.connManager.Connect(myID, identity.FromAddress("unknown-node"), dto2.ConnectOptions{}))
...
func (tc *testContext) TestOnConnectErrorStatusIsNotConnectedAndSessionStartIsNotMarked() {
	...
	assert.Error(tc.T(), tc.connManager.Connect(myID, activeProviderID, dto2.ConnectOptions{}))
	assert.Equal(tc.T(), statusNotConnected(), tc.connManager.Status())
...
func (tc *testContext) TestWhenManagerMadeConnectionStatusReturnsConnectedStateAndSessionId() {
	err := tc.connManager.Connect(myID, activeProviderID, dto2.ConnectOptions{})
...
func (tc *testContext) TestStatusReportsConnectingWhenConnectionIsInProgress() {
	...
	go func() {
		tc.connManager.Connect(myID, activeProviderID, dto2.ConnectOptions{})
...
func (tc *testContext) TestConnectResultsInAlreadyConnectedErrorWhenConnectionExists() {
	assert.NoError(tc.T(), tc.connManager.Connect(myID, activeProviderID, dto2.ConnectOptions{}))
	assert.Equal(tc.T(), ErrAlreadyExists, tc.connManager.Connect(myID, activeProviderID, dto2.ConnectOptions{}))
}
